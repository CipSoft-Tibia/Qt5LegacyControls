...
FocusScope {
    ...
    */
    ...
    Loader {
        ...
        property alias __control: calendar
        property QtObject styleData: null
        onStatusChanged: {
            if (status === Loader.Error)
                console.error("Failed to load Style for", calendar)
        }
    }
    ...
    Loader {
        ...
        onStatusChanged: if (status === Loader.Error) console.error("Failed to load Style for", calendar)
    }

    /*!
        \qmlsignal Calendar::cellHovered(date date)

        Emitted when the mouse hovers over a valid date in the calendar.

        \e date is the date that was hovered over.

        The corresponding handler is \c onCellHovered.
    */
    signal cellHovered(date date)
